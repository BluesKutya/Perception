...
cMainWindow::cMainWindow( ){
	...
	ui.logVerbose      ->setChecked     ( config.logVerbose        );
...
void cMainWindow::on_saveSettings_clicked(){
	...
	config.logVerbose        = ui.logVerbose       ->isChecked();
	...
	if( ret == add ){
		...
		if( d.exec() ){
			...
			if( pd.exec() ){
				...
				cfg.save( config.getGameConfigFile(cfg.exePath) , QList<int>()<<cConfig::SAVE_GAME );
				LoadGames( );
			}
		}
	}
...
void cMainWindow::ScanGames(){
	...
	for( QString& file : file_list ){
		...
		if( !profile.isEmpty() ){
			...
			cfg.save( config.getGameConfigFile(file) , QList<int>()<<cConfig::SAVE_GAME );
		}
	}
